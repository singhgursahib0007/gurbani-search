...
### `min-height: 100dvh` meant the scroll handlers never fired

The app grew to its content, so the *document* scrolled and the panes did not.
Everything looked right — but the navigation bar was listening to a pane that
never moved, so the large-title collapse had never once fired. Fixed with a
fixed viewport, `overflow: hidden`, and `min-height: 0` on flex children.
